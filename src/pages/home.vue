<template>
  <div>
    ...
    <form @submit.prevent="getFiles" class="text-center m-3">
      ...
      />
      <button type="submit" class="border p-2 bg-red-300 rounded">
        submit
      </button>
    </form>
    <button
      class="border p-2 bg-red-400 rounded font-bold"
      @click="saveCurrent"
    >
      Save Current Path
    </button>
    ...
    <div class="flex items-center justify-center">
      ...
      <select
        name="location"
        id="location"
        class="border rounded p-1 border-gray-300 my-2"
        @change="navigate"
      >
    ...
    <input
      type="text"
      placeholder="Search..."
      class="border-2 p-2 rounded text-2xl block mx-auto"
    />
    <div class="flex flex-wrap items-center my-5">
      <a
        href="javascript:void(0)"
        v-for="(folder, i) in location"
        :key="folder"
        class="hover:underline"
        @click="updateContents(getLocation(i + 1))"
      >
        <span class="flex">
  ...
  methods: {
    ...
    updateContents(value) {
      ...
      fetch(`http://localhost:3000/contents?path=${value}`).then(async resp => {
        let { files, folders } = await resp.json();
        this.contents = {
          files: files.sort(),
    ...
    folderClick({ target }) {
      this.updateContents(
        `${this.getLocation()}/${target.children[1].textContent}`
      );
    },
    ...
    navigate({ target: { value } }) {
      +value > -1 &&
        this.updateContents(
          JSON.parse(localStorage.getItem("locHistory"))[+value]
        );
    }
  }
};
